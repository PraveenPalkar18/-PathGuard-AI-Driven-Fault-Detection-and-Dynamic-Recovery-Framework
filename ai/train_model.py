...
def auto_label(
    ...
    """
    Create binary labels for each row in the monitoring dataset.
    ...
    loss_threshold : float
        Maximum packet-loss percentage considered NORMAL.
    rtt_percentile : int
        Percentile of ``rtt_avg_ms`` and ``rtt_mdev_ms`` above which
        readings are considered anomalous.  Using a data-driven
        threshold makes the labelling adaptive to different topologies.

    Returns
    -------
    pd.Series
        Series of ``"NORMAL"`` / ``"FAULT"`` strings.
    """
    # ── Adaptive latency thresholds ──────────────────────────────
    # We use the Pth-percentile of the healthy subset (low loss)
    # as the latency ceiling.  This automatically adapts to the
    # baseline latency of the topology under test.
    healthy_mask = df["packet_loss_pct"] < loss_threshold
    healthy_subset = df.loc[healthy_mask]

    if healthy_subset.empty:
        # Edge case: all rows look like faults — fall back to global stats
        warnings.warn(
            "No rows with packet_loss < threshold; using global percentiles."
        )
        healthy_subset = df

    rtt_avg_threshold  = healthy_subset["rtt_avg_ms"].quantile(
        rtt_percentile / 100.0
    )
    rtt_mdev_threshold = healthy_subset["rtt_mdev_ms"].quantile(
        rtt_percentile / 100.0
    )

    # Ensure thresholds are at least a sensible minimum so the model
    # doesn't flag every single measurement as FAULT
    rtt_avg_threshold  = max(rtt_avg_threshold,  1.0)
    rtt_mdev_threshold = max(rtt_mdev_threshold, 0.5)

    print(f"  Auto-label thresholds:")
    print(f"    packet_loss_pct  < {loss_threshold:.1f}%")
    print(f"    rtt_avg_ms       < {rtt_avg_threshold:.2f} ms  (P{rtt_percentile})")
    print(f"    rtt_mdev_ms      < {rtt_mdev_threshold:.2f} ms  (P{rtt_percentile})")

    # ── Apply labelling rules ────────────────────────────────────
    is_normal = (
        (df["packet_loss_pct"] < loss_threshold)
        & (df["rtt_avg_ms"]    < rtt_avg_threshold)
        & (df["rtt_mdev_ms"]   < rtt_mdev_threshold)
    )

    labels = pd.Series(LABEL_FAULT, index=df.index)
    labels[is_normal] = LABEL_NORMAL
...
def load_dataset(
    ...
    """
    Load the monitoring CSV, validate columns, and create labels.
    ...
    if len(df) < 10:
        raise ValueError(
            f"Dataset too small ({len(df)} rows after cleaning). "
            f"Run the monitor for more rounds to collect sufficient data."
        )

    # ── Feature matrix & labels ──────────────────────────────────
    features = df[FEATURE_COLUMNS].copy()
    labels = auto_label(df, loss_threshold, rtt_percentile)
    df["label"] = labels
...
def train(
    ...
    le = LabelEncoder()
    y = le.fit_transform(labels)   # FAULT=0, NORMAL=1 (alphabetical)
    X = features.values
    ...
    print(f"  {'':12s}  {'Pred FAULT':>12s}  {'Pred NORMAL':>12s}")
    for i, cls_name in enumerate(le.classes_):
        print(f"  {('True ' + cls_name):12s}  {cm[i][0]:12,}  {cm[i][1]:12,}")
...
class FaultDetector:
    ...
    def predict_advanced(
        ...
        # Dynamic Severity & Explainable AI
        severity = "NORMAL"
        explanation = "Traffic is stable"

        if packet_loss_pct >= 50.0:
            severity = "CRITICAL"
            explanation = "Severe packet loss observed"
        elif packet_loss_pct > 0.0:
            severity = "WARNING"
            explanation = "Intermittent loss detected"
        elif rtt_mdev_ms >= 15.0:
            severity = "WARNING"
            explanation = "Link instability pattern detected"
        elif rtt_avg_ms >= 50.0:
            severity = "WARNING"
            explanation = "High RTT spike detected"
        elif base_label == "FAULT":
            severity = "WARNING"
            explanation = "Multiple degraded metrics detected"
    ...
    def predict_batch_advanced(self, df: pd.DataFrame) -> List[Dict]:
...
def run_sample_predictions(detector: FaultDetector):
    ...
    for desc, loss, avg, mx, mdev in samples:
        ...
        colour = "\033[92m" if label == LABEL_NORMAL else "\033[91m"
        reset  = "\033[0m"
